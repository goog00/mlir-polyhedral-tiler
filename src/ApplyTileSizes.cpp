...
static void collectPerfectLoopBand(affine::AffineForOp rootLoop,
    ...
    while (true) {
        ...
        for (Operation &operation : *bodyBlock) {
            if (isa<affine::AffineYieldOp>(operation)) continue;
            ++nonTerminatorCount;
...
/// 若属性不存在, 返回空向量表示不进行分块.
static SmallVector<int64_t, 8> readTileSizesForBand(ArrayRef<affine::AffineForOp> band) {
    ...
    if (!band.empty()) {
        ...
        if (auto arrayAttr = outer->getAttrOfType<ArrayAttr>("tile.sizes")) {
            if (parseArray(arrayAttr)) goto normalize;
        }
    }
...
//
static FailureOr<affine::AffineForOp> stripMineSingleAffineFor(affine::AffineForOp oldFor,
                                                               int64_t tileSize) {
    if (!oldFor || tileSize <= 1) return failure();
    ...
    affine::AffineForOp outerFor = builder.create<affine::AffineForOp>(
        loc, oldLowerOperands, oldLowerMap, oldUpperOperands, oldUpperMap, outerStep);
    ...
    Value vPlus =
        bodyBuilder.create<affine::AffineApplyOp>(loc, addCMap, ValueRange{outerInduction});

    // ubVal = apply old upper map
    Value ubVal =
        bodyBuilder.create<affine::AffineApplyOp>(loc, oldUpperMap, ValueRange(oldUpperOperands));
    ...
    // 用 IntegerSet 表达: (d1 - d0) >= 0 也就是 ubVal - vPlus >= 0.
    {
        ...
        auto ifOp = bodyBuilder.create<affine::AffineIfOp>(loc, condSet, ValueRange{vPlus, ubVal},
                                                           /*withElseRegion=*/true);
        ...
        // 维度 d0 对应外层 %V, 不需要也不能把 %V 当成 symbol 传入.
        {
            ...
            AffineMap ubDimPlus = AffineMap::get(
                /*dims=*/1, /*syms=*/0, d0i + thenBuilder.getAffineConstantExpr(outerStep));

            // 维度实参传入外层 induction 变量
            ValueRange dimOps{outerInduction};

            affine::AffineForOp innerFull =
                thenBuilder.create<affine::AffineForOp>(loc,
        ...
        // 注意: 仍保持 1 个维度 (对应外层 %V), 虽然表达式里未用到 d0.
        {
            ...
            AffineMap ubSymId =
                AffineMap::get(/*dims=*/1, /*syms=*/1, elseBuilder.getAffineSymbolExpr(0));

            // 维度与符号操作数: 先传外层维度, 再传 ubVal 这个 symbol
            SmallVector<Value, 2> ubOps{outerInduction, ubVal};
            ValueRange lbOps{outerInduction};

            affine::AffineForOp innerTail =
                elseBuilder.create<affine::AffineForOp>(loc,
...
static bool applyStripMiningToBand(ArrayRef<affine::AffineForOp> band,
    ...
    for (int level = static_cast<int>(band.size()) - 1; level >= 0; --level) {
        ...
        FailureOr<affine::AffineForOp> result = stripMineSingleAffineFor(target, tileSize);
        if (succeeded(result)) {
            changed = true;
...
/// 并应用 strip-mining.
struct ApplyTileSizesPass : public PassWrapper<ApplyTileSizesPass, OperationPass<func::FuncOp>> {
    ...
    void runOnOperation() override {
        ...
        for (auto root : roots) {
            ...
            if (tileSizes.empty()) continue;

            // 打印 band 概览
            llvm::errs() << "[apply-tiles] Band depth = " << band.size() << ", tile.sizes = [";
            for (unsigned i = 0; i < tileSizes.size(); ++i) {
                llvm::errs() << tileSizes[i];
                if (i + 1 < tileSizes.size()) llvm::errs() << ", ";
            }
            llvm::errs() << "]\n";
    ...
    return {MLIR_PLUGIN_API_VERSION,
            ...
            []() {
                ...
                ::mlir::PassPipelineRegistration<>(
                    "gpu-apply-tiles", "Strip-mining tiling for affine bands with scf inner loops.",
                    [](::mlir::OpPassManager &pm) { pm.addPass(createApplyTileSizesPass()); });
            }};
}
